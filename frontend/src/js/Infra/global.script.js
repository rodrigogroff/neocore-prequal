...
var animateText = function () {
  ...
  $("[data-transation]").each(function (e, i) {
    ...
    let transation = that.attr("data-transation");
    if (transation == "") transation = "fadeInDown";

    let startTime = parseInt(that.attr("data-start-time"));
...
$(document).on("click", ".expandable-item .expandable-header", function () {
  if ($(this).parent().hasClass("accordion")) {
    if ($(this).parent().hasClass("active")) {
      ...
      let accordionGroup = $(this).parent().attr("data-group");
...
$(document).on("click", ".tab-item .menu-item", function (e) {
  e.preventDefault();
  let tabContentId = $(this).attr("data-content");
...
$(document).on("click", "[data-popup]", function () {
  let modalId = $(this).attr("data-popup");
  fadeIn($("#" + modalId)[0], 50);
});
